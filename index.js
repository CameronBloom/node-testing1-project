...
function trimProperties(obj) {
  const copy = { ...obj }
  for (let properties in copy) {
    copy[properties] = copy[properties].trim()
  }
  return copy
}
...
class Counter {
  ...
  countDown() {
    // ✨ implement
    if (this.number > 0) {
      return this.number-- // post decrement
    }
    return this.number
  }
}
...
class Seasons {
  ...
  next() {
    // ✨ implement
    const season = this.seasons[this.curr];
    if (this.curr === 3) {
      this.curr = 0;
    } else {
      this.curr++
